    ...
    >
      <div class="modal-content max-w-md">
        <div class="modal-header">
          <h2 class="modal-title">Choose Species Options</h2>
          <button @click="handleCancel" class="modal-close">×</button>
        </div>
        <div class="modal-body">
          <div class="mb-4">
            <h3 class="text-base font-semibold mb-2">{{ speciesName }}</h3>
            <p class="text-sm text-[var(--color-text-secondary)] mb-4">{{ speciesDescription }}</p>
          </div>

          <div
            v-for="(choice, choiceKey) in speciesChoices"
            :key="choiceKey"
            class="mb-4"
          >
            ...
            <div v-else-if="choice.type === 'subrace'" class="flex flex-col gap-2">
              <button
                v-for="option in choice.options"
                :key="option"
                @click="selectChoice(choiceKey, option)"
...
const speciesData = computed(() => {
  ...
  const speciesDataJson = speciesJson as { species: any[] }
  return speciesDataJson.species.find(s => s.id === props.speciesId) || null
})

const speciesName = computed(() => speciesData.value?.name || '')
const speciesDescription = computed(() => speciesData.value?.description || '')
const speciesChoices = computed(() => speciesData.value?.choices || {})

const canConfirm = computed(() => {
  // Check if all required choices have been made
  const choices = speciesChoices.value
...
const getLineageName = (lineageId: string): string => {
  if (!speciesData.value || !speciesData.value.lineages) return lineageId
  const lineage = speciesData.value.lineages[lineageId]
  return lineage?.name || lineageId
}
...
// Reset choices when modal opens/closes or species changes
